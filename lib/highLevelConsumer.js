...
HighLevelConsumer.prototype.connect = function () {
    ...
    this.on('ready', function () {
        ...
        self.fetchOffset(self.topicPayloads, function (err, topics) {
            ...
            //console.log('topicPayloads', self.topicPayloads);
            //console.log('GETTING INITIAL OFFSETS', topics);
            // for the first request, we want to start from the value of the offset present in Zookeeper
...
/*
 * Update offset info in current payloads (this is the request)
...
/*
 * Progress offset info in current payloads by 1 (this is the request)
 * @auth brenley
 * @param {Object} Topic-partition-offset
 * @param {Boolean} Don't commit when initing consumer
 */
HighLevelConsumer.prototype.progressOffsets = function (topics, initing) {
    this.topicPayloads.forEach(function (p) {
        if (!_.isEmpty(topics[p.topic]) && topics[p.topic][p.partition] !== undefined)
...
function autoCommit(force, cb) {
    ...
    var commits = this.topicPayloads.reduce(function (out, p) {
        if (p.offset !== 0)
            out.push(_.defaults({ offset: p.offset }, p));
        return out;
    }, []);
